...
## Prérequis
- Node.js 18+
- Backend SUPER-VIDEOTHEQUE opérationnel (par défaut http://localhost:4000)

## Installation
```bash
cd frontend
cp .env.sample .env   # ajustez VITE_API_BASE_URL si besoin
npm install
npm run dev
```
...
- `npm run build` : compilation prod (tsc + vite build).
...
| Nom | Description |
| --- | --- |
| `VITE_API_BASE_URL` | URL du backend (ex: `http://localhost:4000`). |
...
3. Persist de la session/rentals (localStorage) + gestion offline.
4. Tests Vitest/RTL pour les hooks et composants critiques.
